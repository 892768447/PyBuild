...
public class jarsigner {
	...
	public static void main(String[] zArgs) {
		try {
			...
			// }
			//
			// if (keystoreUrl == null) throw new
			// IllegalArgumentException("Unable to locate keystore " +
			// keystore);
			// zipSigner.signZip(keystoreUrl, keystoreType, keystorePass,
			// keyAlias, keyPass, inputFile, outputFile);

			/*
			 * if(zArgs.length>2){ zipsigner.setKeymode(zArgs[2]); }else{
			 ...
			 * 
			 * public void onProgress(ProgressEvent event) { String message =
			 * event.getMessage(); int percentDone = event.getPercentDone(); //
			...
			Logger.getLogger(jarsigner.class.getName()).log(Level.SEVERE, null,
					ex);
